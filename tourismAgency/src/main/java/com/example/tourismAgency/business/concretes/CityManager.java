...
public class CityManager implements CityService {
	...
	@Autowired
	public CityManager(CityRepository cityRepository, MapperService mapperService) {
	...
	public DataResult<GetCityResponse> getCityById(int id) {
		GetCityResponse cityResponse = this.mapperService.forResponse().map(cityRepository.getReferenceById(id), GetCityResponse.class);
	...
	public DataResult<List<GetAllCityResponse>> getAll() {
		List<GetAllCityResponse> getAll = this.cityRepository.findAll().stream().map(item -> this.mapperService.forResponse().map(item, GetAllCityResponse.class)).collect(Collectors.toList());
